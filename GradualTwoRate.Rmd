...
In the data from the 60 degree group, it could be that the bootstrapped fits from the abrupt condition, can not explain the gradual data  (MSE distribution on purple line) compared to how well the gradual fits explain the gradual data (yellow line). The difference is in the black line. Let's have a look at the 95% confidence intervals:

```{r}
crossConditionMSEs(exp=1)
```

Zero difference falls outside the confidence interval, so that we now look at the model parameter values to see if there is any difference in the parameter values:

```{r fig.width=7, fig.height=4}
plotExpModelParameters(exp=1)
```

The distributions of parameter values overlap very well, let's look at the confidence intervals of the differences:

```{r}
conditionParameterDiffs(exp=1)
```

Zero is included in all confidence intervals, so that we can not conclude that parameters are different between the two conditions.
...
```{r fig5, fig.width=4, fig.height=7}
...
For reach deviations in the rotated block, across all three groups in the MCEBAT experiment, there is no real support for the null hypothesis, but there is in the clamped block. What about the separate groups in exp 2?
```{r}
conditionBayesFactors(groups=getInfo()$group[which(getInfo()$experiment==2)])
```

These evidence is less strong for each group, but notice that all Bayes Factors are below 1, so that more powerfull experiments (or larger data sets) would probably lower them. The combined data is fairly clear anyway.
...
Let's see the confidence interval of the cross-fit quality:

```{r}
crossConditionMSEs(exp=2)
```

So zero is not included in the 95% confidence interval in all groups, which means that model fits are not equivalent between conditions (the parameters from the abrupt condition do not predict the gradual condition as well as the parameters fit on the gradual data). This means we will look at the parameter values themselves.

```{r}
plotExpModelParameters(exp=2)
```

The lines depict the distribution of bootstrapped parameters and the dots the parameter values for individual participants. We will check the confidence intervals of the differences in parameter values:

```{r}
conditionParameterDiffs(exp=2)
```

In the two older groups (mild cerebellar ataxia, and their age-matched controls) the 95% confidence interval of differences in values of parameter `Lf` between abrupt and ramped conditions does not include zero. This shows that in the ramped condition, these groups have a higher learning rate for the fast process. However the slow process shows no differences. In the younger group, the parameters are also equivalent.

# Conclusion

What does this mean? The models show a difference, in parameter values between conditions. However, this difference is not in the slow process, but the fast process. Furthermore, if the fast process reflects explicit learning, than we would expect the fast learning rate to be decreased in the ramped condition (that is supposed to evoke more implicit learning), but we find the opposite. Since there are no behavioral differences, this might simply be an oddity of the model rather than reflect any actual neural/adaptation processes.

An alternative explanation is that the perturbation schedule used in the second experiment leads to lower parameter recoverability, and perhaps accidental differences in fits.
